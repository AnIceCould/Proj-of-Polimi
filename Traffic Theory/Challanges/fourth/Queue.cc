...
protected:
    ...
    simsignal_t processedCountSignal;

    double avgServiceTime;
    int processedCount; // 计数器：记录已完成的消息数
...
void Queue::initialize()
{
    ...
    processedCountSignal = registerSignal("processedCount");

    //initial messages
    emit(qlenSignal, queue.getLength());
    emit(busySignal, serverBusy);
    emit(processedCountSignal, processedCount);
...
void Queue::startPacketService(cMessage *msq)
{
    ...
    if(processedCount % 3 == 0){
        ...
        scheduleAt(simTime()+serviceTime + 0.15, endOfServiceMsg);
    }
    else{
        simtime_t serviceTime = exponential(avgServiceTime);
