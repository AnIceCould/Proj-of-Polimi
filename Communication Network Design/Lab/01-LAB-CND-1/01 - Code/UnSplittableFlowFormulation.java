...
public class UnSplittableFlowFormulation implements IAlgorithm
{
	...
	DoubleMatrix1D demandTrafficVector = netPlan.getVectorDemandOfferedTraffic();
	DoubleMatrix2D demandTrafficMatrix = demandTrafficVector.like2D(D, E);
	for (int e = 0; e < E; e++) {
	    demandTrafficMatrix.viewColumn(e).assign(demandTrafficVector);
	}
	op.setInputParameter ("V_c" , demandTrafficMatrix);
	...
	for (Link lk : netPlan.getLinks ()) {
		...
		op.addConstraint ("sum(V_c(all, lk) .* x_lkc(all, lk)) <= linkCapacity");
	}
